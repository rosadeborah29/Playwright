...
test(`Handling dropdown in select tag`, async ({page}) => {
    ...
    const dropdownallValues = await page.locator('//select[@id="createLeadForm_industryEnumId"]/option').allInnerTexts();
    const dropdownCount = dropdownallValues.length; // Number of webeelement present in the dropdown
    for (let index = 0; index <dropdownCount; index++) {
       console.log (dropdownallValues[index]);
    }
    ...
    await page.waitForTimeout(3000) //Only for demo purpose
})